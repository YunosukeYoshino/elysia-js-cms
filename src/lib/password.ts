 ...
 * - 後方互換性を考慮した バージョニング
 */
export async function hashPassword(password: string): Promise<{ hash: string; version: string }> {
  try {
    // パスワード強度検証
    ...
    throw new Error(
      `パスワードハッシュ化に失敗しました: ${error instanceof Error ? error.message : error}`,
    );
  }
}
 ...
 * 後方互換性のため維持
 */
async function verifyLegacyPassword(_password: string, _hashedPassword: string): Promise<boolean> {
 ...
 * 既存システムからの移行用
 */
async function verifyLegacySimplePassword(
  _password: string,
